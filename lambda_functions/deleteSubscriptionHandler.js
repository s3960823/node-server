...
module.exports = function deleteSubscriptionHandler(req, res) {
  ...
  if (!username || !title || !year) {
    return res
      .status(400)
      .json({ error: "Username, title, and year are required" });
  }
  ...
  dynamodb.deleteItem(params, (err, data) => {
    if (err) {
      ...
      if (err.code === "ConditionalCheckFailedException") {
        return res
          .status(404)
          .json({ error: "Item not found or title does not match" });
      }
    ...
    res
      .status(200)
      .json({ message: "Item deleted successfully from subscription table" });
  });
};
